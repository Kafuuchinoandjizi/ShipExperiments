...
// 配置选项：选择百分位数计算方法
// 0 = Quick Select (精度高，速度中)
// 1 = 直方图法 (速度最快，精度略低 ~0.05%)
#define PERCENTILE_METHOD 1

float get_percentile(ImageF *img, float p) {
#if PERCENTILE_METHOD == 0
...
// 融合：减法 + 乘法
// output[i] = (input[i] - beta) * scale
static inline void fused_sub_mul(const float *input, float *output, int size, 
...
// 融合：减法（矩阵）
// output[i] = a[i] - b[i]
static inline void fused_sub(const float *a, const float *b, float *output, int size) {
    #pragma omp parallel for
    for (int i = 0; i < size; ++i) {
        output[i] = a[i] - b[i];
    }
...
#pragma omp parallel for
    ...
    double processing_time1 = (double)(end_time1 - start_time1) / CLOCKS_PER_SEC;
    ...
    double processing_time2 = (double)(end_time2 - start_time2) / CLOCKS_PER_SEC;
...
#pragma omp parallel for
    ...
    double processing_time3 = (double)(end_time3 - start_time3) / CLOCKS_PER_SEC;
...
#pragma omp parallel for
    ...
    double processing_time4 = (double)(end_time4 - start_time4) / CLOCKS_PER_SEC;
    ...
    double processing_time5 = (double)(end_time5 - start_time5) / CLOCKS_PER_SEC;
...
void process_directory(const char *root_dir, const char *output_root_dir) {
    ...
    int guided_filter_radius = 8;  // 原值: 16
    ...
    do {
        ...
        printf("Processed: %s -> %s.bmp (Total Time: %.4f s)\n", find_data.cFileName, stem, processing_time);
