...
// Optimized percentile with smaller histogram for speed
float get_percentile(ImageF *img, float p) {
    int size = img->width * img->height;

    // Reduced histogram size for faster processing (512 bins is enough)
#define HIST_BINS 512
    int histogram[HIST_BINS] = {0};

    float min_val = FLT_MAX, max_val = -FLT_MAX;

    // Fast min/max with NEON
#ifdef __ARM_NEON
    ...
    // Build histogram - reduce thread overhead with larger chunks
...
ImageU8* enhanced_thermal_conversion(ImageF* image_14bit, int radius, float eps,
    ...
    // Step 1: Normalize
    ImageF *image_norm = create_image_f_fpga(w, h);
    ...
    for (int i = 0; i < size; i += 4) {
        if (i + 4 <= size) {
            ...
            vst1q_f32(image_norm->data + i, vmulq_f32(v, v_norm));
        } else {
            for (int j = i; j < size; j++)
                image_norm->data[j] = image_14bit->data[j] * norm_factor;
        }
    }
#else
#pragma omp parallel for num_threads(NUM_THREADS) schedule(static, 256)
    for (int i = 0; i < size; ++i)
        image_norm->data[i] = image_14bit->data[i] * norm_factor;
#endif
    ...
    for (int i = 0; i < size; i += 4) {
        if (i + 4 <= size) {
            float32x4_t v1 = vld1q_f32(image_norm->data + i);
            float32x4_t v2 = vld1q_f32(base_layer->data + i);
            vst1q_f32(detail_layer->data + i, vsubq_f32(v1, v2));
        } else {
            for (int j = i; j < size; j++)
                detail_layer->data[j] = image_norm->data[j] - base_layer->data[j];
        }
    }
#else
#pragma omp parallel for num_threads(NUM_THREADS) schedule(static, 256)
    for (int i = 0; i < size; ++i)
        detail_layer->data[i] = image_norm->data[i] - base_layer->data[i];
#endif
...
#pragma omp parallel for num_threads(NUM_THREADS) schedule(static, 256)
    ...
    ImageF *base_comp = create_image_f_fpga(w, h);
...
#pragma omp parallel for num_threads(NUM_THREADS) schedule(static, 256)
    ...
    ImageF *fused = create_image_f_fpga(w, h);
...
#pragma omp parallel for num_threads(NUM_THREADS) schedule(static, 256)
    ...
    for (int i = 0; i < size; i += 4) {
        if (i + 4 <= size) {
            ...
            for (int j = i; j < size; j++) {
                float val = (fused->data[j] - fmin) * scale;
                if (val < 0) val = 0;
                if (val > 255) val = 255;
...
#pragma omp parallel for num_threads(NUM_THREADS) schedule(static, 256)
    for (int i = 0; i < size; ++i) {
        float val = (fused->data[i] - fmin) * scale;
        if (val < 0) val = 0;
        if (val > 255) val = 255;
    ...
    free_image_f_fpga(image_norm);
    free_image_f_fpga(base_layer);
    free_image_f_fpga(detail_layer);
    free_image_f_fpga(base_log);
    free_image_f_fpga(base_comp);
    free_image_f_fpga(fused);
